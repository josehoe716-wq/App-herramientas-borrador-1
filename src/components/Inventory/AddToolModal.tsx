...
import { X, Package, Upload, Image } from 'lucide-react';
import { Tool } from '../../types';

interface AddToolModalProps {
  onClose: () => void;
  onAdd: (tool: Omit<Tool, 'id' | 'created_at' | 'updated_at'>) => Promise<any>;
}

export const AddToolModal: React.FC<AddToolModalProps> = ({ onClose, onAdd }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await onAdd({
      ...formData,
      available_stock: formData.stock,
    });
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-4">
                <label className="flex items-center space-x-2 bg-blue-50 hover:bg-blue-100 px-4 py-2 rounded-lg cursor-pointer border border-blue-200 transition-colors">
                  ...
                  <span className="text-sm text-blue-700">Subir Imagen</span>
                  <input
                    type="file"
                    accept="image/*"
